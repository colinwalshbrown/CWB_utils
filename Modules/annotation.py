...
import sqlite
import pickle
from fasta_subseq_2 import FastaDB

class GenomeAnnotation:
    
    def __init__(annotation,name=None,sequence=None,reindex=False):
        self.annFile = annotation
        self.format = format
        self.name = name
        self.sequence = sequence
        self.annDBs = pickle.load(open(os.environ["HOME"] + "/scripts/python_modules/ANNOTATION_INDEX_DBS"))
        if (self.alnFile):
