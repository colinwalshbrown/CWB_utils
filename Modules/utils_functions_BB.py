...
##################
# From 1 position in a genome, gives the corresponding position in another genome using a whole genome aln.
...
# Input: sps1 (original), sps2 (destination), chr1,pos_g1 (position in sps1), map_folder,genomes_folder,dict_fragment (dictionary of fragments, if not used before, you must create a dictionary. This is to avoid to reload the same aln fragments several times if several aln positions are to be searched). 
# Returns location in sps2.
##################
def PosGenome1_2_PosGenome2 (sps1, chr1, pos_g1, lmap, map_folder, genomes_folder, dict_fragment, sps2, chr2, pos_g2):
	(fragment, pos_a, orient1) = genomePosition2alnPosition(sps1, chr1, pos_g1, lmap, map_folder, genomes_folder, dict_fragment)
	sps_pos2=FindsSpsPosInAln(sps2, genomes_folder)
	if fragment=="NA":
		return ("NA", "NA", "NA")
	else:
		(chr_fragment, beg_fragment, end_fragment, orient2)=retrieveFragmentInfo (sps_pos2, lmap, fragment, fragment, "same")
		(chr2, pos_g2) = alnPosition2genomePosition(sps2, fragment, pos_a, orient2, lmap, map_folder, genomes_folder, dict_fragment)
		return (chr2, pos_g2, orient2)


#####################
# Opens the map file and puts it in a list
#####################
def MakesMapList(map_folder, lmap):
	try:
		fmap=open(map_folder+"/map", 'r')
	except IOError, e:
		print "Unknown map: ",map_folder+"/map"
		sys.exit()
	lmap=list()
	n=0
	NAs=list()
	for l in fmap: # finds the good alignment fragment.
		liste=l.split()
		if n==0:
			for i in range(len(liste)):
				...
				n=n+1
		liste[0]=int(liste[0])
		if liste[0]==len(lmap)+1: # there is no fragment missing (list index and fragment number are the same)
			lmap.append(liste)	
		elif liste[0] > len(lmap)+1: # 1 fragment is missing, I am adding a list of "NA" at the place of the missing fragment
			lmap.append(NAs)
		elif liste[0] < len(lmap)+1:
			print "Pb with the map file: a fragment list is not well ordered."
			sys.exit()
	print "The map contains "+str(len(lmap))+" fragments."
	return (lmap)
...
#################
def retrieveFragmentInfo(sps_pos, lmap, i, fragment, param): # param says whether fragment was given for i and fragment entries ("same"). If not, param="NOTsame")
#	print ">>>> "+str(fragment)
	debut=4*sps_pos
	if fragment!="NA":
		if param=="same":
...
#		print fragment
#		print lmap[i]
#		print ">> "+str(lmap[i][debut+2])
		fragment=int(fragment)
		if str(i).isdigit():
			if lmap[i][0] != "NA":
				if fragment == int(lmap[i][0]): # chooses the good alignment file fragment to open
					chr_fragment=lmap[i][debut+1]
					if chr_fragment != "NA":
						...
						(beg_fragment, end_fragment, orient)=("NA","NA","NA")
				else:
					(chr_fragment, beg_fragment, end_fragment, orient)=("NA", "NA","NA","NA")
			else:
				(chr_fragment, beg_fragment, end_fragment, orient)=("NA", "NA","NA","NA")
		else:
			(chr_fragment, beg_fragment, end_fragment, orient)=("NA", "NA","NA","NA")
		return(chr_fragment, beg_fragment, end_fragment, orient)
	else:
		return("NA","NA","NA","NA")
...
def CompleteFragmentDict (dict_fragment, fragment, map_folder, format):
	if format=="fasta":
		...
		ext=".aln"
	frag_handle = open(map_folder+"/"+str(fragment)+ext, "rU")
...
def alnpos2seqpos (dict_fragment, fragment, sps, beg_frag, end_frag, orientation, pos_aln): # returns the position in the aln corresponding to the position in the genome.
	if (pos_aln=="NA") or (not str(pos_aln).isdigit()):
		return("NA")
	else:
		for record in range(len(dict_fragment[fragment])): #[1,2,3] if no "NA". Less otherwise.
			if sps in dict_fragment[fragment][record].id:
				n=0
				for i in range(len(dict_fragment[fragment][record].seq)):
					if dict_fragment[fragment][record].seq[i] != "-":
...
##################
## From 1 position in a genome, finds the corresponding aln fragment.
##################
def genomePosition2fragment (sps, chr, pos_g, lmap, map_folder, genomes_folder, dict_fragment):
	fragment = "NA"
...
## Finds good aln fragment and coordinates of the fragment in the aln.
	fragment=genomePosition2fragment(sps, chr, pos_g, lmap, map_folder, genomes_folder, dict_fragment)
	if fragment!="NA":
		beg_fragment=""
		end_fragment=""
		orient=""
		(chr_fragment, beg_fragment, end_fragment, orient)= retrieveFragmentInfo(sps_pos, lmap, fragment, fragment, "same")										
...
# finds the infos for the aln fragment
	for i in range(len(lmap)):	
		if (fragment!="NA") and (lmap[i][0]!="NA") and ((int(lmap[i][0]) == int(fragment))): # "NA" may appear in the case of aln of more than 2 sps.
			chr_fragment, beg_fragment, end_fragment, orient= retrieveFragmentInfo (sps_pos, lmap, i, fragment, "NOTsame")
			chr = lmap[i][debut+1] # chooses the good chr
			break
										
	# opens the aln fragment and screens it to find the good coordinate pos_a.
	if (fragment!="NA") and (lmap[i][0]!="NA"):
		handle = open(map_folder+"/"+str(fragment)+".mfa", "rU")
